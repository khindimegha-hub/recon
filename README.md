...
The Streamlit dashboard provides a visual view of the reconciliation run, including match coverage, classification results, AI investigations, human escalations, and transaction-level details.

![Recon Dashboard](screenshot/dashboard.png)

The dashboard is generated from the same reconciliation output and audit JSON used by the pipeline, so the displayed numbers can be validated against the actual source output.
